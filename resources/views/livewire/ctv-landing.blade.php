<div class="min-h-screen bg-[#050505] text-white selection:bg-blue-500/30 font-sans tracking-tight overflow-x-hidden">
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex items-center justify-between backdrop-blur-md bg-black/20 border-b border-white/5">
        <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.5)]">
                <i class="fa-solid fa-cloud-bolt text-white text-sm"></i>
            </div>
            <span class="text-xl font-black tracking-tighter uppercase italic">Antigravity</span>
        </div>
        <div class="flex items-center gap-8">
            <a href="#about" class="text-sm font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-colors">Về chúng tôi</a>
            <a href="#ranking" class="text-sm font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-colors">Vinh danh</a>
            <a href="/login" class="px-6 py-2 rounded-full border border-blue-500/50 text-blue-400 text-sm font-bold uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all shadow-[0_0_15px_rgba(59,130,246,0.2)]">Đăng nhập</a>
        </div>
    </nav>
    ...
    <section class="relative min-h-screen flex flex-col items-center justify-center pt-20 px-6 overflow-hidden">
        ...
        <div class="relative z-10 text-center max-w-4xl mx-auto">
            ...
            <h1 class="text-6xl md:text-8xl font-black leading-none mb-8 tracking-tighter bg-gradient-to-b from-white via-white to-white/40 bg-clip-text text-transparent">
                NÂNG TẦM <br/>
                <span class="bg-gradient-to-r from-blue-400 to-indigo-600 bg-clip-text text-transparent">KINH DOANH</span> <br/>
                KHÔNG TRỌNG LỰC
            </h1>
            ...
            <div class="flex flex-col md:flex-row items-center justify-center gap-6">
                <a href="/login?register=1" class="group relative px-10 py-5 bg-blue-600 rounded-2xl text-white font-black uppercase tracking-widest overflow-hidden transition-all hover:shadow-[0_0_40px_rgba(37,99,235,0.6)]">
                ...
                <a href="#ranking" class="px-10 py-5 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md text-white font-black uppercase tracking-widest hover:bg-white/10 transition-all">
                    Xem bảng xếp hạng
                </a>
            </div>
        </div>
        ...
        <!-- Floating Elements -->
        <div class="absolute bottom-20 left-10 md:left-20 animate-float">
    ...
    <section id="ranking" class="py-32 bg-gradient-to-b from-transparent to-white/[0.02]">
        <div class="container mx-auto px-6">
            ...
            <div class="max-w-5xl mx-auto space-y-6">
                ...
                <div class="group relative flex items-center justify-between p-8 rounded-[38px] bg-white/[0.02] border border-white/5 transition-all hover:bg-white/5 hover:border-blue-500/30 hover:-translate-y-1 {{ $index == 0 ? 'bg-blue-500/[0.03] border-blue-500/20 shadow-[0_0_50px_rgba(59,130,246,0.1)]' : '' }}">
                    <div class="flex items-center gap-8">
                        <div class="relative">
                            <div class="text-4xl font-black {{ $index == 0 ? 'text-yellow-400' : ($index == 1 ? 'text-gray-300' : ($index == 2 ? 'text-orange-400' : 'text-gray-700')) }} w-12 text-center">
                                {{ $index + 1 }}
                            </div>
                            @if($index < 3)
                            <div class="absolute -top-4 -left-4 text-xs animate-bounce opacity-50">
                                <i class="fa-solid fa-crown text-yellow-500"></i>
                            </div>
                            @endif
                        </div>
                        <div class="w-16 h-16 rounded-2xl bg-gradient-to-tr {{ $index == 0 ? 'from-yellow-400 to-orange-500' : 'from-blue-500 to-indigo-600' }} flex items-center justify-center text-2xl font-black shadow-lg">
                            {{ substr($performer->name, 0, 1) }}
                        </div>
                        <div>
                            <div class="flex items-center gap-3">
                                <div class="text-xl font-black uppercase tracking-tight">{{ $performer->name }}</div>
                                @if($performer->rank)
                                <span class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-tighter bg-blue-500/20 text-blue-400 border border-blue-500/30">
                                    {{ $performer->rank->name }}
                                </span>
                                @endif
                            </div>
                            <div class="text-xs text-gray-500 font-mono tracking-widest mt-1">ID: {{ $performer->id }} • {{ substr($performer->phone, 0, 7) }}***</div>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-xs text-gray-400 uppercase font-black mb-1">Doanh số</div>
                        <div class="text-2xl font-mono text-blue-400">{{ number_format($performer->total_revenue, 0, ',', '.') }} <span class="text-xs text-gray-600 font-sans">VND</span></div>
                    </div>
    ...
    <footer class="py-40 text-center relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-3xl mx-auto px-6">
            <h2 class="text-5xl font-black mb-8 tracking-tighter italic">SẴN SÀNG KHAI PHÁ <br/> TÌM NĂNG CÙNG ANTIGRAVITY?</h2>
            <p class="text-gray-400 mb-12 font-medium">Đăng ký tài khoản ngay hôm nay để nhận mã mời và bắt đầu xây dựng đế chế bất động sản của riêng bạn.</p>
            
            <a href="/register" class="inline-flex items-center gap-4 px-12 py-6 bg-white text-black rounded-3xl font-black uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all hover:shadow-[0_0_50px_rgba(59,130,246,0.3)]">
                Đăng ký tài khoản <i class="fa-solid fa-arrow-right"></i>
            </a>
